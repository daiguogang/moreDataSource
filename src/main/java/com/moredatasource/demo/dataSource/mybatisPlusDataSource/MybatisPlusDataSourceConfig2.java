 ...
 * @see MybatisPlusDataSourceConfig2
 */
@Configuration
@MapperScan(basePackages = "com.moredatasource.demo.dataSource.mybatisPlusDataSource.mybatisPlusDao2", sqlSessionTemplateRef = "plusDs1SqlSessionTemplate")
public class MybatisPlusDataSourceConfig2 {

    // @Primary 表示这个数据源是默认数据源
    @Primary
    // @Bean 将返回的这个对象放到spring容器中
    @Bean("plusDs2SqlSessionFactory")
    ...
    public SqlSessionFactory ds1SqlSessionFactory(@Qualifier("ds2DataSource") DataSource dataSource) throws Exception {
        ...
        sqlSessionFactory.setMapperLocations(new PathMatchingResourcePatternResolver().
                getResources("classpath*:mybatisPlusMapper2/**/*.xml"));
...
//        sqlSessionFactory.setGlobalConfig(new GlobalConfig().setBanner(false));
    ...
    // 事务支持
    @Primary
    @Bean(name = "plusDs2TransactionManager")
    public DataSourceTransactionManager ds1TransactionManager(@Qualifier("ds2DataSource") DataSource dataSource) {
        return new DataSourceTransactionManager(dataSource);
    }

    @Primary
    @Bean(name = "plusDs2SqlSessionTemplate")
    // 获取mybatis 的核心对象 SqlSessionTemplate
    public SqlSessionTemplate ds1SqlSessionTemplate(@Qualifier("plusDs2SqlSessionFactory") SqlSessionFactory sqlSessionFactory) {
